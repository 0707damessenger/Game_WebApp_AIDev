...
function renderHand(handElement, player) {
...
export function renderApp(elements, state, localPlayerId) {
  ...
  renderBoard(elements.board, state);
  renderPlayers(elements.playerList, state, localPlayerId);
  renderHand(elements.hand, localPlayer);
  ...
  elements.statusMessage.textContent = active.id === localPlayerId ? '轮到你行动' : `等待${active.label}行动`;
