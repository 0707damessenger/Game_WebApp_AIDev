...
**Architecture:** 使用纯前端 ES Modules。`pomodoro-state.mjs` 只处理状态转换和规则校验，不访问浏览器 API；`app.mjs` 负责计时、通知与事件派发；`render.mjs` 根据状态重绘唯一的模拟桌面悬浮餐车窗口及其菜单。所有可调数值、地点和模式选项只来自唯一的 `CONFIG`。

**Tech Stack:** 原生 HTML/CSS/JavaScript ES Modules、Node.js 内置测试运行器、Playwright、Microsoft Edge（现有本机浏览器）。

---

> **实施修订（验收反馈）：** 本计划最初以“普通窗口和小窗切换”为界面方案。该方案已被验收反馈替代：当前原型与正式设计均采用**单一悬浮餐车窗口 + 窗口内菜单**，并在工作或休息结束时显示非阻塞阶段提示。具体规则以 `design.md` 为准；下方早期双窗口代码片段仅保留为已执行过程记录，不代表当前实现。
...
- 原型始终只显示一个模拟桌面悬浮餐车窗口；安排、番茄钟、设置和后续管理功能通过窗口内菜单进入，不尝试系统级置顶。
